...
def construct_waltz():
    music_files = []
    for i in range(16):
        mmid = minuet_table[i][roll_dice(2)]
        music_files.append(minuet_filename(mmid))
    for i in range(16):
        tmid = trio_table[i][roll_dice(1)]
        music_files.append(trio_filename(tmid))
    for filename in music_files:
        #wave_obj = simpleaudio.WaveObject.from_wave_file(filename)
        #play_obj = wave_obj.play()
        #print(os.path.abspath('.') + "/mozart_project/mozart/")
        with wave.open(os.path.abspath('.') + "/mozart_project/mozart/" + filename, 'rb') as f:
            params = f.getparams()
            nchannels, sampwidth, framerate, nframes = params[:4]
            nframes = int(nframes)
            strData = f.readframes(nframes)
            waveData = np.frombuffer(strData, dtype='float32')
        play_obj = simpleaudio.play_buffer(waveData, nchannels, sampwidth,framerate)
        play_obj.wait_done()
    return -1
