...
def write_to_excel():
    ...
    url = 'http://datachart.500.com/pls/history/inc/history.php?limit=15116&start=04001&end=04011'
    html = get_html(url)  # 调用自定义函数,读网页获取网页内容
    print('正在提取保存数据......')
    if html is not None:        # 如果读网页没出错,读成功,则进行下一步,
        # 写入每一期信息
        '''
